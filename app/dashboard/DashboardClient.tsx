...
import { calcMonth, calcTotals, fmtShort, type Month } from '@/lib/calc'
...
export default function DashboardClient({
  ...
  const [months, setMonths] = useState<Month[]>(
    initialMonths.map(m => ({ ...m, brokers: m.brokers || [], expenses: Number(m.expenses) }))
  )
  ...
  months.forEach(m => m.brokers.forEach(b => {
    brokerMap[b.name] = (brokerMap[b.name] || 0) + Number(b.payout)
  }))
  const brokerList = Object.entries(brokerMap).sort((a, b) => b[1] - a[1])

  function openAdd() {
    setForm({ label: '', brokers: [{ id: crypto.randomUUID(), name: '', payout: '' }], expenses: '' })
    setModal('add')
  }
  ...
  async function saveAdd() {
    ...
    try {
      const { data: newMonth, error: mErr } = await supabase
        .from('months').insert({ label: form.label, expenses: Number(form.expenses), user_id: userId }).select().single()
      if (mErr) throw mErr
      const { data: newBrokers, error: bErr } = await supabase.from('brokers')
        .insert(form.brokers.map((b: any) => ({ name: b.name, payout: Number(b.payout), month_id: newMonth.id, user_id: userId }))).select()
      if (bErr) throw bErr
      setMonths(p => [...p, { ...newMonth, brokers: newBrokers, expenses: Number(newMonth.expenses) }])
      setModal(null)
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const { data: newBrokers } = await supabase.from('brokers')
        .insert(form.brokers.map((b: any) => ({ name: b.name, payout: Number(b.payout), month_id: modal, user_id: userId }))).select()
      setMonths(p => p.map(m => m.id === modal ? { ...form, brokers: newBrokers, expenses: Number(form.expenses) } : m))
      setModal(null)
    } catch (err) { console.error(err) } finally { setSaving(false) }
  }
  ...
  const preview = form ? calcMonth({
    brokers: form.brokers.map((b: any) => ({ ...b, payout: Number(b.payout) || 0 })),
    expenses: Number(form.expenses) || 0,
  }) : null

  const inp = {
    width: '100%', background: '#0d1a0d', border: '1px solid #0d2b0d',
    ...
    <div style={{ minHeight: '100vh', background: '#060808', color: '#e2e8f0', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; }
        ...
        ::-webkit-scrollbar-track { background: #060808; }
      ...
      <div style={{ background: '#080d08', borderBottom: '1px solid #0d2b0d', padding: '0 28px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ fontSize: 11, color: '#1a4a1a', display: 'flex', gap: 8 }}>
            <span>Boss split <span style={{ color: '#00cc6a', fontWeight: 700 }}>30%</span></span>
            <span style={{ color: '#0d2b0d' }}>·</span>
            <span>Aron split <span style={{ color: '#a78bfa', fontWeight: 700 }}>50%</span></span>
          </div>
          <button onClick={openAdd} style={{ background: 'transparent', border: '1px solid #00ff88', color: '#00ff88', fontFamily: "'DM Sans',sans-serif", fontWeight: 700, fontSize: 12, padding: '8px 18px', cursor: 'pointer', borderRadius: 8, letterSpacing: 0.5 }}>
            + PŘIDAT MĚSÍC
          </button>
          <button onClick={logout} style={{ background: 'transparent', border: '1px solid #0d2b0d', color: '#1a4a1a', fontFamily: "'DM Sans',sans-serif", fontSize: 12, padding: '7px 14px', cursor: 'pointer', borderRadius: 8 }}>
            Odhlásit
          </button>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '28px' }}>
        ...
        <div style={{ marginBottom: 28, animation: visible ? 'up 0.5s ease both' : 'none' }}>
          ...
          <div style={{ fontSize: 72, fontWeight: 800, color: '#00ff88', letterSpacing: -3, lineHeight: 1, textShadow: '0 0 40px rgba(0,255,136,0.4), 0 0 80px rgba(0,255,136,0.15)' }}>
            {fmtShort(totals.mine)}
          </div>
          <div style={{ display: 'flex', gap: 28, marginTop: 14, flexWrap: 'wrap' }}>
            {[
              ['Celkový payout', fmtShort(totals.payout), '#374151'],
              ['Tvůj 30%', fmtShort(totals.myShare), '#1a5a3a'],
              ['Expenses', fmtShort(totals.expenses), '#4a1a1a'],
              ['Aronovi', fmtShort(totals.aron), '#3a1a5a'],
              [`⌀ ${fmtShort(avg)}/měs`, '', '#1a4a1a'],
            ].map(([l, v, c]) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 24 }}>
          {[
            ...
            { label: 'Tvůj podíl 30%', value: fmtShort(totals.myShare), color: '#00cc6a', border: '#0d2b1a' },
            { label: 'Aronovi 50%', value: fmtShort(totals.aron), color: '#a78bfa', border: '#1a0d2b' },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 16, marginBottom: 24 }}>
          <div style={{ background: '#080d08', border: '1px solid #0d2b0d', borderRadius: 14, padding: '22px', animation: visible ? 'up 0.4s ease 0.28s both' : 'none' }}>
            ...
            {allStats.length === 0 ? (
              <div style={{ textAlign: 'center', color: '#1a4a1a', padding: '40px 0' }}>Přidej první měsíc</div>
            ) : (
          ...
          <div style={{ background: '#080d08', border: '1px solid #0d2b0d', borderRadius: 14, padding: '22px', animation: visible ? 'up 0.4s ease 0.35s both' : 'none' }}>
            ...
            {brokerList.length === 0 ? (
              <div style={{ color: '#1a4a1a', fontSize: 12 }}>Zatím žádné účty</div>
            ) : (
            ...
            <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid #0d2b0d' }}>
              <div style={{ fontSize: 10, color: '#1a4a1a', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 12 }}>Dělení tvého podílu</div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, fontSize: 11 }}>
                <span style={{ color: '#00ff88', fontWeight: 700 }}>Ty 50%</span>
        ...
        {allStats.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(270px, 1fr))', gap: 14 }}>
            {allStats.map((m, i) => (
              <div key={m.id} className="mcard" onClick={() => openEdit(m)}
                style={{ background: '#080d08', border: '1px solid #0d2b0d', borderRadius: 14, padding: '20px', animation: visible ? `up 0.4s ease ${0.4 + i * 0.07}s both` : 'none' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 14 }}>
                  {m.brokers.map(b => (
                    <div key={b.id} className="brow" style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 8px', borderRadius: 6, background: '#0d1a0d' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
                  <div><div style={{ fontSize: 9, color: '#1a4a1a', marginBottom: 2 }}>Tvůj 30%</div><div style={{ fontSize: 12, fontWeight: 700, color: '#00cc6a' }}>{fmtShort(m.myShare)}</div></div>
      ...
      {modal && form && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, animation: 'fadeIn 0.2s ease' }} onClick={() => setModal(null)}>
          <div style={{ background: '#080d08', border: '1px solid #0d2b0d', borderRadius: 16, padding: '32px', width: '100%', maxWidth: 520, maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 0 60px rgba(0,255,136,0.08)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 11, color: '#1a4a1a', marginBottom: 28 }}>Payout → ×30% → −expenses → −50% Aron = čistý zisk</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 10 }}>
              {form.brokers.map((b: any, i: number) => (
                <div key={b.id} style={{ display: 'grid', gridTemplateColumns: '1fr 110px 34px', gap: 8 }}>
                  ...
                  <input type="number" placeholder="$ payout" value={b.payout} onChange={e => setForm((f: any) => ({ ...f, brokers: f.brokers.map((x: any, j: number) => j === i ? { ...x, payout: e.target.value } : x) }))} style={{ ...inp, color: '#00ff88', fontWeight: 700 }} />
            ...
            <button onClick={() => setForm((f: any) => ({ ...f, brokers: [...f.brokers, { id: crypto.randomUUID(), name: '', payout: '' }] }))}
              ...
              <div style={{ background: '#0a0f0a', border: '1px solid #0d2b0d', borderRadius: 10, padding: '16px 18px', marginBottom: 24 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {[
                    ...
                    ['Tvůj podíl (30%)', fmtShort(preview.myShare), '#00cc6a'],
                    ['− Expenses', `−${fmtShort(Number(form.expenses || 0))}`, '#f87171'],
                    ['− Aronovi (50%)', `−${fmtShort(preview.aronAmt)}`, '#a78bfa'],
                  ].map(([l, v, c]) => (
